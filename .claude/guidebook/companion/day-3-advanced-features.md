...
### Option 3: Cross-Platform Notification Sync

**What to build:**

Synchronize notification read status across web and mobile devices in real-time.

**Requirements:**

- Notification sync service (backend)
- WebSocket event broadcasting to all user devices
- Mark as read on any device → updates all other devices
- Real-time badge and UI updates across platforms
- Requires mobile app foundation (Day 4 content)

**Validation:**

- [ ] Mark notification read on web → mobile badge updates
- [ ] Mark notification read on mobile → web badge updates
